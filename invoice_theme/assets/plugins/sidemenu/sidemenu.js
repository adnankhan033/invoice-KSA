(function () {
    ...
    // Toggle Sidebar
    jQuery(document).on('click', '[data-bs-toggle="sidebar"]', function (event) {
        event.preventDefault();
        jQuery('.app').toggleClass('sidenav-toggled');
    });
    responsive();
   // clearing the clicking functions already present on the element
   jQuery("[data-bs-toggle='slide']").off('click');
   jQuery("[data-bs-toggle='sub-slide']").off('click');
   jQuery("[data-bs-toggle='sub-slide2']").off('click');

   // initiating the click function
   jQuery("[data-bs-toggle='slide']").on('click', function (e) {
   ...
   jQuery("[data-bs-toggle='sub-slide']").on('click', function (e) {
       var jQuerythis = jQuery(this);
       var checkElement = jQuerythis.next();
       var animationSpeed = 300,
   ...
   // Activate sidebar slide toggle
   jQuery("[data-bs-toggle='sub-slide2']").on('click', function (e) {
       var jQuerythis = jQuery(this);
       var checkElement = jQuerythis.next();
       var animationSpeed = 300,
   ...
   jQuery('.hor-content').on('click', function () {
       jQuery('.side-menu li').each(function () {
           jQuery('.side-menu ul.open').slideUp(300)
           jQuery(this).parent().removeClass("is-expanded");
           jQuery(this).parent().parent().removeClass("open");
           jQuery(this).parent().parent().prev().removeClass("is-expanded");
           jQuery(this).parent().parent().parent().removeClass("is-expanded");
           jQuery(this).parent().parent().parent().parent().removeClass("open");
           jQuery(this).parent().parent().parent().parent().parent().removeClass("is-expanded");
       })
   })

    var position = window.location.pathname.split('/');
    jQuery(".app-sidebar li a").each(function () {
        var jQuerythis = jQuery(this);
        var pageUrl = jQuerythis.attr("href");
        ...
        if (pageUrl) {
            if (position[position.length - 1] == pageUrl) {
                ...
                jQuery(this).parent().parent().prev().addClass("is-expanded");
                ...
                jQuery(this).parent().parent().parent().parent().parent().addClass("is-expanded");
                return false;
            }
        }
    });
    if (jQuery('.slide-item').hasClass('active')) {
        jQuery('.app-sidebar').animate({
            scrollTop: jQuery('a.slide-item.active').offset().top - 600
        }, 600);
    }
    if (jQuery('.sub-slide-item').hasClass('active')) {
        jQuery('.app-sidebar').animate({
            scrollTop: jQuery('a.sub-slide-item.active').offset().top - 600
        }, 600);
    }
    ...
    jQuery(window).on("scroll", function (e) {
        if (jQuery(window).scrollTop() >= 70) {
            jQuery('.app-header').addClass('fixed-header');
            jQuery('.app-header').addClass('visible-title');
        } else {
            jQuery('.app-header').removeClass('fixed-header');
            jQuery('.app-header').removeClass('visible-title');
        }
    });

    jQuery(window).on("scroll", function (e) {
        if (jQuery(window).scrollTop() >= 70) {
            jQuery('.horizontal-main').addClass('fixed-header');
            jQuery('.horizontal-main').addClass('visible-title');
        } else {
            jQuery('.horizontal-main').removeClass('fixed-header');
            jQuery('.horizontal-main').removeClass('visible-title');
        }
    });
})();
...
function hovermenu() {
    ...
    jQuery(".app-sidebar").hover(function () {
        if (jQuery('.app').hasClass('sidenav-toggled')) {
            jQuery('.app').addClass('sidenav-toggled-open');
        }
    }, function () {
        if (jQuery('.app').hasClass('sidenav-toggled')) {
            jQuery('.app').removeClass('sidenav-toggled-open');
        }
    });
}
...
function hovermenu() {
    jQuery(".app-sidebar").hover(function () {
        if (jQuery('body').hasClass('sidenav-toggled')) {
            jQuery('body').addClass('sidenav-toggled-open');
        }
    }, function () {
        if (jQuery('body').hasClass('sidenav-toggled')) {
            jQuery('body').removeClass('sidenav-toggled-open');
        }
    });
}
...
function icontext() {
    jQuery(".app-sidebar").off("mouseenter mouseleave");

    jQuery(document).on('click', ".app-sidebar", function (event) {
        if (jQuery('body').hasClass('sidenav-toggled') == true) {
            jQuery('body').addClass('sidenav-toggled-open');
        }
    });
    ...
    var alterClass = function () {
        ...
        if (ww < 992) {
            ...
            jQuery('body').addClass('sidenav-toggled');
        };
    };
    ...
    () => {
        ...
        if (window.innerWidth >= 992 && prevValue < 992 || window.innerWidth >= 992) {
            if (document.querySelector('body').classList.contains('horizontal')) {
                ...
                var animationSpeed = 300;
                // first level
                var parent = jQuery("[data-bs-toggle='sub-slide']").parents('ul');
                var ul = parent.find('ul:visible').slideUp(animationSpeed);
                ul.removeClass('open');
                var parent1 = jQuery("[data-bs-toggle='sub-slide2']").parents('ul');
            ...
            if ((window.innerWidth < 992 &&  prevValue > 992 )|| document.querySelector('body').classList.contains('horizontal') != true){
                if (pageUrl) {
                    if (position[position.length - 1] == pageUrl) {
                        ...
                        jQuery(this).parent().parent().parent().parent().prev().addClass("active");
                        jQuery(this).parent().parent().parent().parent().parent().addClass("is-expanded");
                        jQuery(this).next().slideDown(300, function () {});jQuery(this).parent().parent().slideDown(300, function (){
                            jQuery(this).parent().parent().addClass("open");
                        });
                        jQuery(this).parent().parent().parent().parent().slideDown(300, function (){
                            jQuery(this).parent().parent().parent().parent().addClass("open");
                        });
                        return false;
                    }
...
function checkHoriMenu() {
    ...
    if(menuItems.scrollWidth - 2 < (menuWidth?.offsetWidth - menuContainerWidth)){
        jQuery("#slide-right").addClass("d-none");
        jQuery("#slide-left").addClass("d-none");
    }
    else if(marginLeftValue != 0 ){
        jQuery("#slide-left").removeClass("d-none");
    }
    else if(marginLeftValue != -check){
        jQuery("#slide-right").removeClass("d-none");
    }
...
checkHoriMenu();
jQuery(document).on("click", ".ltr #slide-left", function () {
    let menuWidth = document.querySelector('.horizontal-main')
    let menuItems = document.querySelector('.side-menu')
    let mainSidemenuWidth = document.querySelector('.main-sidemenu')    
    let menuContainerWidth = menuWidth?.offsetWidth - mainSidemenuWidth?.offsetWidth
...
jQuery(document).on("click", ".ltr #slide-right", function () {
...
jQuery(document).on("click", ".rtl #slide-left", function () {
    let menuWidth = document.querySelector('.horizontal-main')
    let menuItems = document.querySelector('.side-menu')
    let mainSidemenuWidth = document.querySelector('.main-sidemenu')    
    let menuContainerWidth = menuWidth?.offsetWidth - mainSidemenuWidth?.offsetWidth
...
jQuery(document).on("click", ".rtl #slide-right", function () {
    let menuWidth = document.querySelector('.app-sidebar')
    let menuItems = document.querySelector('.side-menu')
    let mainSidemenuWidth = document.querySelector('.main-sidemenu')
    let menuContainerWidth = menuWidth?.offsetWidth - mainSidemenuWidth?.offsetWidth
